-- Attero Tech unA6IO-BT Plugin for Q-Sys 5.3+
-- Attero Tech unA6IO-BT Plugin for Q-Sys 5.3+
...
PluginInfo =
{
  Name = "Attero Tech~AES67~unA6IO-BT",
  Version = "1.0.0",
  Id = "1d67a62d-8d4e-4384-b685-726434008969",
  Description = "Control plugin for Attero Tech unA6IO-BT",
}

function GetPrettyName(props)
  return string.format("Attero Tech unA6IO-BT %s", PluginInfo.Version)
end
...
function GetControlLayout(props)
  ...
  layout = {
    ...
    BTName =
    {
      PrettyName = "Device~Bluetooth Name",
      Style = "Text",
      TextBoxStyle = "Normal",
      Position = { 144, 70 },
      Size = { 124 , 16 },
    ...
    BTNameApply =
    {
      PrettyName = "Device~Bluetooth Apply Name",
      Style = "Button",
      Legend = "Apply",
      ButtonStyle = "Momentary",
      Color = Color.Orange,
    ...
    BTBtnLock =
    {
      PrettyName = "Device~Bluetooth Pair Btn",
      Style = "Button",
      Legend = "Enabled",
      ButtonStyle = "Toggle",
      Color = Color.Orange,
    ...
    BTBtnPair =
    {
      PrettyName = "Device~Bluetooth Start Pair",
      Style = "Button",
      Legend = "Pair / Connect",
      ButtonStyle = "Momentary",
      Color = Color.Orange,
    ...
    {
      Style = "Text",
      PrettyName = "Device~Bluetooth Status",
      TextBoxStyle = "NoBackground",
      IsReadOnly = true,
      Position = { 144 , 110 },
      Size = { 162 , 16 },
  ...
  for i=1,2 do
    ...
    layout["InputSelect "..i] = {
      ...
      Position = { ((i - 1) * CH_X_OFFSET) + 316 , 70 },
      Size = { 76 , 16 },
      Fill = Color.White,
      TextSize = 10,
    }
  ...
  graphics = 
  {
    ...
    --Bluetooth input config Groupbox
    {
      Type = "GroupBox",
      Text = "Bluetooth",
      Position = { 140, 10 },
    ...
    --Bluetooth TX Horizontal Divider
    {
      Type = "GroupBox",
      Position = { 144, 46 },
      Size = { 156 , 1 },
    ...
    --Bluetooh Pair Button Ctrl Label
    {
      Type = "Label",
      Text = "Front Panel Pair Button",
      Position = { 144 , 90 },
      Size = { 100 , 16 },
      Color = Color.White,
      TextSize = 10,
...
editBTName = Controls.BTName
btnBTNameApply = Controls.BTNameApply
btnBTBtnLock = Controls.BTBtnLock
btnBTBtnPair = Controls.BTBtnPair
editBTStatus = Controls.BTStatus
...
local PROD_ID          = "0x0000000000000010"
...
local commErrCnt = 0
local localIP = ""
local mtr_ip = ""   --Added for local storage as no visible controls for this value
local mtr_prt = 0   --Added for local storage as no visible controls for this value
local sta_prd = 0   --Added for local storage as no visible controls for this value
local mtr_prd = 0   --Added for local storage as no visible controls for this value
local btStatus = 0  --Global storgae for current BT state
...
function SetControlState(boolEnable)
  ...
  editBTName.IsDisabled      = not boolEnable or (tonumber(btStatus) == 1)
  btnBTNameApply.IsDisabled  = not boolEnable or (tonumber(btStatus) == 1)
  btnBTBtnLock.IsDisabled    = not boolEnable or (tonumber(btStatus) == 1)
  btnBTBtnPair.IsDisabled    = not boolEnable or (tonumber(btStatus) == 1)
  editBTStatus.IsDisabled    = not boolEnable or (tonumber(btStatus) == 1)
  ...
  if ack == "ACK" then
    ...
    if cmd == sentCmd then
      ...
      elseif cmd == "QUERY" then
        local id, ov1, ov2, om1, om2, is3, is4, btn, bts, btl = param:match("ID=(%a+) OV1=(%-?%d+) OV2=(%-?%d+) OM1=(%a+) OM2=(%a+) IS3=(.+) IS4=(.+) BTN=(.+) BTS=(.+) BTL=(.+)")
        --print("Query: "..DebugFormat(param))
        --print("unA6IO-BT:", id, ov1, ov2, om1, om2, is3, is4, btn, bts, btl)

        if id ~= nil then
          editBTName.String = btn
          oldBTName = btn
          btStatus = bts
          editBTStatus.String = "BT Status: "..BT_STATUS[tonumber(bts) + 1]
          btnBTBtnLock.Boolean = (btl == "0")
      ...
      elseif cmd == "BTS" then
        --print("BTS: "..DebugFormat(param, true))
        btStatus = param
        editBTStatus.String = "BT Status: "..BT_STATUS[tonumber(param) + 1]
        SetControlState(ledConnected.Boolean)   --Make sure BT controls change state based on new value
      elseif cmd == "BTN" then
        oldBTName = editBTName.String   --set background name if name succesfully changed
      elseif cmd == "IS" or cmd == "BTB" or cmd == "BTL" or cmd == "ID" or cmd == "OV" or cmd == "OM" or cmd == "SAVE" then
    ...
    if ack == "NACK" then
      if cmd == "BTN" then  --if BTN command fails, revert to prevsiously good name.
        editBTName.String = oldBTName
      end
...
--* Description - Checks responses from BIN messages
--********************************************************************************
...
--********************************************************************************
--* Function: BTNameApplyBtnEvent(ctl) 
--* Description: Event Handler for "Refresh" button press
--********************************************************************************
function BTNameApplyBtnEvent(ctl) 
  if( ctl.Boolean ) then    --Only do this when button pushed 
    editBTName.String = string.gsub(editBTName.String, "%s+", "")   --remove all spaces
    if ( string.len(editBTName.String) > 0 ) then    --Make sure name has some text
      if ( string.len(editBTName.String) > MAX_BT_NAME_LEN ) then    --If text is too long, truncate it.
        editBTName.String = string.sub(editBTName.String, 1, MAX_BT_NAME_LEN)
      end
      AddCmdToQueue("ASCII", "BTN", editBTName.String)
    else
      editBTName.String = oldBTName
    end
  end
...
--********************************************************************************
--* Function: BTBtnLockBtnEvent(ctl) 
--* Description: Event Handler for "Refresh" button press
--********************************************************************************
function BTBtnLockBtnEvent(ctl) 
  AddCmdToQueue("ASCII", "BTL", tostring(ctl.Boolean and 0 or 1))
end


--********************************************************************************
--* Function: BTPairBtnEvent(ctl) 
--* Description: Event Handler for "Refresh" button press
--********************************************************************************
function BTPairBtnEvent(ctl) 
  if( ctl.Boolean ) then    --Only do this when button pushed 
    statusTimer:Start(3)    --Delay next Status update to prevent miss readings
    healthTimer:Start(5)    --Delay next Health timer update to prevent miss readings
    AddCmdToQueue("ASCII", "BTB")

    --force controls to be disabled
    editBTName.IsDisabled      = true
    btnBTNameApply.IsDisabled  = true
    btnBTBtnLock.IsDisabled    = true
    btnBTBtnPair.IsDisabled    = true
    editBTStatus.IsDisabled    = true
  end
end
...
--********************************************************************************
--* Function : statusTimer.Eventhandler
--* Description: Event Handler for "statusTimer" time out event
--********************************************************************************
statusTimer.EventHandler = function()
  if(btnConnect.Boolean == true) then   --Only do something if Conenction should be active
    AddCmdToQueue("ASCII", "BTS")
    statusTimer:Start(1)
  end
end
...
function StartupCode()
  ...
  btnBTNameApply.EventHandler = BTNameApplyBtnEvent
  btnBTBtnPair.EventHandler = BTPairBtnEvent
  btnBTBtnLock.EventHandler = BTBtnLockBtnEvent
